...
// "Transposition Table" : previous move choices for previously seen board situations
#define PM_NB       (1 << 24)
...
struct prev_move_t prev_moves[PM_NB];
...
int get_prev_move(int depth, int side)
{
    struct move_t move;
    ...
    // look if it is in the board hashes table. If not add it
    int h = hash % PM_NB;
    if (prev_moves[h].hash != hash) goto new_entry;
    if (prev_moves[h].depth < depth) goto new_entry;
    move = prev_moves[h].move;
    if ((B(move.from) & COLORS) != side) goto new_entry;
    if (B(move.to) != move.eaten) goto new_entry;
    return h;

new_entry:
    prev_moves[h].hash = hash;
    prev_moves[h].flag = 0;
    nb_hash++;
    return h;
}
...
int nega_alpha_beta(int level, int a, int b, int side, struct move_t *upper_sequence)
{
    ...
    int h     = -1;
    if (level > 1 && level < 8) {
        h    = get_prev_move(depth, side);
        eval = prev_moves[h].eval;
        flag = prev_moves[h].flag;
        ...
        if (flag == EXACT_VALUE || a >= b) {
            nb_dedup++;
            mm_move          = prev_moves[h].move;
    ...
    // then try first the move from transposition table (likely to be the best)
    else if (flag) {
        if (prev_moves[h].move.val) mm_move = prev_moves[h].move;
    }
    if (mm_move.val) {
        do_move(mm_move);
        ab_moves++;
...
end_add_to_tt:
    if (h >= 0) {
        prev_moves[h].eval     = max;
        prev_moves[h].move.val = mm_move.val;
        prev_moves[h].depth    = depth;
        if (max <= old_a) prev_moves[h].flag = UPPER_BOUND;
        else if (max >= b) prev_moves[h].flag = LOWER_BOUND;
        else prev_moves[h].flag = EXACT_VALUE;
    }
...
void compute_next_move(void)
{
    ...
    memset(prev_moves, 0, sizeof(prev_moves));
